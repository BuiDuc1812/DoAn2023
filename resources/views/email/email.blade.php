        ...
        <div class="invoice-container" ref="document" id="html">
           <table style="width:100%; height:auto;  text-align:center; " BORDER=0 CELLSPACING=0>
             <thead style="background:#fafafa; padding:8px;">
               <tr style="font-size: 20px;">
                 <td colspan="4" style="padding:20px 20px;text-align: left;">{{ $data['subject']}}</td>
               </tr>
             </thead>
             <tbody style="background:#ffff;padding:20px;">
               <tr>
                 <td colspan="4" style="padding:20px 0px 0px 20px;text-align:left;font-size: 16px;color:#000;">Mã đơn hàng <b>#120{{ $data['order']->id }}</b> Từ
                    <b>{{ $data['order']->created_at->diffForHumans() }}</b>   </td>
               </tr>
               <tr>
                <td colspan="4" style="padding:20px 0px 0px 20px;text-align:left;font-size: 16px;color:#000;">Họ tên khách hàng: {{ $data['order']->user->name }}</td>
              </tr>
              <tr>
                <td colspan="4" style="padding:20px 0px 0px 20px;text-align:left;font-size: 16px;color:#000;">Số điện thoại: {{ $data['order']->phone }}</td>
              </tr>
               <tr>
           ...
           <table style="width:100%; height:auto; background-color:#fff;text-align:center; padding:10px; background:#fafafa">
             <tbody>
               ...
               <tr style="background-color:#fff; font-size:12px; color:#262626;">
                 <td style="border-right:1.5px dashed  #DCDCDC ;width:25%; font-weight:bold;background: #fafafa;">{{ $data['order']->created_at}}</td>
                 <td style="border-right:1.5px dashed  #DCDCDC ;width:25% ; font-weight:bold;background: #fafafa;">#120{{ $data['order']->id }}</td>
                 <td style="border-right:1.5px dashed  #DCDCDC ;width:25%; font-weight:bold;background: #fafafa;">CASH</td>
                 <td style="width:25%; font-weight:bold;background: #fafafa;">{{ $data['order']->address }}</td>
               </tr>
             </tbody>
           </table>
           ...
           <table style="width:100%; height:auto; background-color:#fff;padding:20px; font-size:12px; border: 1px solid #ebebeb; border-top:0">
             ...
             <tfoot style="padding-top:20px;font-weight: bold;">
               <tr>
                 <td style="padding-top:20px;">Contact us <span style="color:#c61932"> 0386075101 </span></td>
               </tr>
             </tfoot>
           </table> 
      </div>
